...
export const refresh = catchAsync(async function (req: Request, res: Response) {
  ...
  if (existingToken.revoked) throw new AppError("Token revoked", 401);
...
// -------------------------- LOGOUT ------------------------------
export const logout = catchAsync(async function (req: Request, res: Response) {
  const refreshToken = req.cookies?.refreshToken;
...
export const resetPassword = catchAsync(async function (
  ...
  if (!password) throw new AppError("Please Enter Your new password", 400);

  const user = await User.findOne({ passwordResetToken: token });

  if (!user) throw new AppError("Token is Expired", 400);
